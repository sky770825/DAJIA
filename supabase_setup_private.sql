-- ============================================
-- 大甲鎮瀾宮官方聯名系列 - Supabase 數據庫設置
-- 使用 PRIVATE schema（資料夾）
-- ============================================
-- 請在 Supabase SQL Editor 中執行此文件
-- 訪問：https://supabase.com/dashboard/project/cnzqtuuegdqwkgvletaa
-- ============================================

-- ============================================
-- 0. 創建 PRIVATE schema（如果不存在）
-- ============================================
CREATE SCHEMA IF NOT EXISTS PRIVATE;

-- ============================================
-- 1. 創建 leads 表（客戶登記）
-- ============================================
CREATE TABLE IF NOT EXISTS PRIVATE.leads (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  name TEXT NOT NULL,
  phone TEXT NOT NULL,
  email TEXT NOT NULL,
  product_interest TEXT NOT NULL,
  usage TEXT,
  quantity TEXT,
  contact_preference TEXT,
  note TEXT,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- 創建索引
CREATE INDEX IF NOT EXISTS idx_leads_created_at ON PRIVATE.leads(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_leads_email ON PRIVATE.leads(email);

-- ============================================
-- 2. 創建 orders 表（訂單）
-- ============================================
CREATE TABLE IF NOT EXISTS PRIVATE.orders (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  order_number TEXT UNIQUE NOT NULL,
  items JSONB NOT NULL,
  total NUMERIC(10, 2) NOT NULL,
  shipping NUMERIC(10, 2) NOT NULL,
  form_data JSONB NOT NULL,
  status TEXT NOT NULL DEFAULT 'pending',
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- 創建索引
CREATE INDEX IF NOT EXISTS idx_orders_created_at ON PRIVATE.orders(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_orders_order_number ON PRIVATE.orders(order_number);
CREATE INDEX IF NOT EXISTS idx_orders_status ON PRIVATE.orders(status);

-- ============================================
-- 3. 創建 verification_codes 表（防偽驗證碼）
-- ============================================
CREATE TABLE IF NOT EXISTS PRIVATE.verification_codes (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  code TEXT UNIQUE NOT NULL,
  product_id TEXT,
  product_name TEXT,
  order_number TEXT,
  status TEXT NOT NULL DEFAULT 'active',
  verified_at TIMESTAMP WITH TIME ZONE,
  verified_count INTEGER DEFAULT 0,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- 創建索引
CREATE INDEX IF NOT EXISTS idx_verification_codes_code ON PRIVATE.verification_codes(code);
CREATE INDEX IF NOT EXISTS idx_verification_codes_order_number ON PRIVATE.verification_codes(order_number);
CREATE INDEX IF NOT EXISTS idx_verification_codes_status ON PRIVATE.verification_codes(status);
CREATE INDEX IF NOT EXISTS idx_verification_codes_created_at ON PRIVATE.verification_codes(created_at DESC);

-- ============================================
-- 4. 設置 Row Level Security (RLS)
-- ============================================

-- 啟用 RLS
ALTER TABLE PRIVATE.leads ENABLE ROW LEVEL SECURITY;
ALTER TABLE PRIVATE.orders ENABLE ROW LEVEL SECURITY;
ALTER TABLE PRIVATE.verification_codes ENABLE ROW LEVEL SECURITY;

-- ============================================
-- 5. 創建安全策略 (Policies)
-- ============================================

-- 刪除現有策略（避免重複創建錯誤）
DROP POLICY IF EXISTS "Allow anonymous insert on leads" ON PRIVATE.leads;
DROP POLICY IF EXISTS "Allow anonymous insert on orders" ON PRIVATE.orders;
DROP POLICY IF EXISTS "Allow anonymous select on verification_codes" ON PRIVATE.verification_codes;
DROP POLICY IF EXISTS "Allow anonymous update verification on verification_codes" ON PRIVATE.verification_codes;
DROP POLICY IF EXISTS "Allow anonymous insert on verification_codes" ON PRIVATE.verification_codes;

-- 允許匿名用戶插入客戶登記數據（前端表單提交）
CREATE POLICY "Allow anonymous insert on leads"
  ON PRIVATE.leads FOR INSERT
  TO anon
  WITH CHECK (true);

-- 允許匿名用戶插入訂單數據（結賬功能）
CREATE POLICY "Allow anonymous insert on orders"
  ON PRIVATE.orders FOR INSERT
  TO anon
  WITH CHECK (true);

-- 允許匿名用戶查詢驗證碼（驗證功能）
CREATE POLICY "Allow anonymous select on verification_codes"
  ON PRIVATE.verification_codes FOR SELECT
  TO anon
  USING (status = 'active');

-- 允許匿名用戶更新驗證碼狀態（記錄驗證）
CREATE POLICY "Allow anonymous update verification on verification_codes"
  ON PRIVATE.verification_codes FOR UPDATE
  TO anon
  USING (status = 'active')
  WITH CHECK (status IN ('active', 'used'));

-- 允許匿名用戶插入驗證碼（訂單創建時）
CREATE POLICY "Allow anonymous insert on verification_codes"
  ON PRIVATE.verification_codes FOR INSERT
  TO anon
  WITH CHECK (true);

-- ============================================
-- 6. 驗證設置（可選 - 用於檢查）
-- ============================================

-- 檢查表是否創建成功
SELECT 
  table_schema,
  table_name,
  table_type
FROM information_schema.tables
WHERE table_schema = 'PRIVATE'
  AND table_name IN ('leads', 'orders', 'verification_codes')
ORDER BY table_name;

-- 檢查索引
SELECT 
  schemaname,
  tablename,
  indexname
FROM pg_indexes
WHERE schemaname = 'PRIVATE'
  AND tablename IN ('leads', 'orders', 'verification_codes')
ORDER BY tablename, indexname;

-- ============================================
-- 完成！
-- ============================================
-- 執行完成後，您可以在 Supabase Dashboard 的 Table Editor 中查看這些表
-- 表位於 PRIVATE schema（資料夾）中
-- 現在可以開始測試網站功能了！
-- ============================================
